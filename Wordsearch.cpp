...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        cerr << "Usage: " << argv[0] << " <grid_file> <words_file>" << endl;
        return 1;
    }

    string gridFilename = argv[1];
    string wordsFilename = argv[2];

    int rows, cols;
    char** grid = loadGrid(gridFilename, rows, cols);
    vector<string> words = loadWords(wordsFilename);
